import{v as b,m as L,w as M,c as x,a as z,Z as D,d as O}from"./elements-DUrmcn_5.js";import{S as R,g as T,b as H,a as E,s as V}from"./seismic-colormap-Bw0ynval.js";import{g as q,j as F}from"./data-D8Ls4_G-.js";import"./preload-helper-PPVm8Dsz.js";import"./_commonjsHelpers-CqkleIqs.js";import"./findsample-BBxmZGgX.js";const P=.1,I=1e3,B=150,Z=30,_=10;function U(t,m){if(!t||t.length===0)return[];const a=t?t.map(h=>[h.easting,h.northing,h.tvd,h.md]):[],s=new b(a,{tension:.75,arcDivisions:5e3}),o=s.length,n=Math.round(o*_);let e;n>0?e=G(s.getPoints(n),5e-4,10):e=[[a[0][0],a[0][1]]];const r=e[0],l=e[e.length-1],u=L.distance(r,l);let i;if(u<B){const j=m/180*Math.PI;i=new L(Math.cos(j),Math.sin(j)).mutable}else i=k(e,Z);const c=Math.max(0,I-o),g=c+o,d=[];let p=[];const v=i.toArray();c>0&&(p=M(Math.ceil(c*P)).map(h=>i.set(v).scale(c*(1-h)).subFrom(r).toArray()),p.pop(),d.push(...p)),d.push(...e);const w=M(Math.ceil(I*P)).map(h=>i.set(v).scale(I*h).add(l).toArray()).splice(1);return d.push(...w),J(d,void 0,g)}function k(t,m){const a=L.zero.mutable;let s=0;const o=L.zero.mutable;for(let n=0;n<t.length-1;n++){const e=t.length-1-n;if(o.set(t[e]).sub(t[e-1]),a.add(o),s=a.magnitude,s>m)break}return s===0?new L([0,0]):a.scale(1/s)}function G(t,m=.001,a=10){if(t.length<=4)return t;const[s,o]=t[0],n=t.map(i=>[i[0]-s,i[1]-o]);let[e,r]=n[0];const l=[t[0]];for(let i=1;i+1<n.length;i++){const[c,g]=n[i]??[],[d,p]=n[i+1]??[];if(c!=null&&g!=null&&d!=null&&p!=null&&(d-c!==0||p-g!==0)){const v=Math.abs(e*p-r*d+d*g-p*c+r*c-e*g)/Math.sqrt((d-e)**2+(p-r)**2),w=[e-c,r-g],C=Math.sqrt(w[0]**2+w[1]**2);(v>m||C>=a)&&(l.push([c+s,g+o]),[e,r]=[c,g])}}const u=n[n.length-1];return l.push([u[0]+s,u[1]+o]),l}function J(t,m,a=0){let s=t[0],o=0;return t.map(e=>{const r=e[0]-s[0],l=e[1]-s[1];return o+=Math.sqrt(r**2+l**2),s=e,[a>0?a-o:o,e[2]??0]})}const f=700,y=600,S=()=>{const t=x(f),m=z(f,y);return Promise.all([q(),F()]).then(a=>{const[s,o]=a,n=new R("seismic",{order:2,layerOpacity:1}),e={elm:m,width:f,height:y};n.onMount({...e});const r=U(s,45),l=T(o,r);H(o,r,V,{isLeftToRight:!0,seismicScale:2}).then(i=>{n.data={image:i,options:E(l)}});const u=new D(m,i=>{n.onRescale(i)});u.setBounds([0,f],[0,y]),u.adjustToSize(f,y),u.setViewport(f/2,y/2,f/2)}),t.appendChild(m),t.appendChild(O()),t},$={title:"ESV Intersection/Features/Seismic",component:S};S.parameters={...S.parameters,docs:{...S.parameters?.docs,source:{originalSource:`() => {
