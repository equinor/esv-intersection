...
import { Completion, foldCompletion, GenericScreen, GenericTubing, OnRescaleEvent } from '../interfaces';
...
export interface CompletionLayerV2Options<T extends Completion[]> extends WellComponentBaseOptions<T> {
  screenScalingFactor?: number;
  screenLineColor?: number;
  tubingScalingFactor?: number;
}

export class CompletionLayerV2<T extends Completion[]> extends WellboreBaseComponentLayer<T> {
  private screenTextureCache: Texture | undefined = undefined;

  private tubingTextureCache: Texture[] = [];

  constructor(ctx: Application | PixiRenderApplication, id: string, options: CompletionLayerV2Options<T>) {
  ...
  preRender(): void {
    ...
    const { screenScalingFactor, tubingScalingFactor, screenLineColor } = this.options as CompletionLayerV2Options<T>;
    this.data.map(
      foldCompletion(
        (obj: GenericScreen) => this.drawGenericScreen(obj, screenScalingFactor, screenLineColor),
        (obj: GenericTubing) => this.drawGenericTubing(obj, tubingScalingFactor),
      ),
    );
  }

  private drawGenericScreen(screen: GenericScreen, scalingFactor: number, lineColor: number): void {
  ...
  private drawGenericTubing(tubing: GenericTubing, scalingFactor: number): void {
  ...
  private createTubularPolygon = (completion: GenericScreen | GenericTubing): TubularRenderingObject => {
    if (completion == null) {
      return;
    }
    ...
    const path = this.getZFactorScaledPathForPoints(completion.start, completion.end, [completion.start, completion.end], zFactor);
  ...
  private createTubingTexture(diameter: number): Texture {
    const { tubingScalingFactor } = this.options as CompletionLayerV2Options<T>;
  ...
  private createScreenTexture(diameter: number): Texture {
    const { screenScalingFactor } = this.options as CompletionLayerV2Options<T>;
