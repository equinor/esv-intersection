...
describe('callout', () => {
  describe('overlap', () => {
    it('should return true for overlap of two text in x- and y-direction', () => {
      const one = {
        x: 0,
        y: 0,
        width: t1Width,
      ...
      const actual = checkOverlap(one, two);
      const actual2 = checkOverlap(two, one);
      expect(actual).toEqual(true);
      expect(actual2).toEqual(true);
    });
    it('should return false for two texts not overlapping in y-direction', () => {
      const one = {
        x: 0,
        y: 0,
        width: t1Width,
    ...
    it('should return false for two texts not overlapping in x-direction', () => {
      const one = {
        x: 0,
        y: 0,
        width: t1Width,
    ...
    it('should highlight three overlapping texts', () => {
      ...
      for (let i = 0; i < arr.length; i++) {
        ...
        for (let j = 0; j < arr.length; j++) {
          const elj = arr[j];
          if (i !== j) {
            const overlapping = checkOverlap(eli, elj);
            if (overlapping) {
