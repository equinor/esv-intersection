...
export const Seismic = () => {
  ...
  generateSeismicSliceImage(seismic, trajectory, seismicColorMap).then((seismicImage: ImageBitmap) => {
    seismicLayer.onUpdate({ ...createEventObj(container), image: seismicImage, options: {x: seismicInfo.minX, y: seismicInfo.minTvdMsl, width: seismicInfo.maxX - seismicInfo.minX, height: seismicInfo.maxTvdMsl - seismicInfo.minTvdMsl } });
  });
